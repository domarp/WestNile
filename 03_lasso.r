...
##=========================================================================================
## Run Model : Lasso
## Run with seeds = 1,2,3,4 and multiply results to get required submission file
##=========================================================================================
for(i in 1:1){
  var.subsample = sample(4:ncol(train.data),0.7* ncol(train.data))
  ...
  for(cross in 1:k){
    ...
    cv = cv.glmnet(as.matrix(train.trn[,2:ncol(train.trn)]), as.matrix(train.trn$WnvPresent),
                   family = "binomial",alpha = 1)
    fit = glmnet(as.matrix(train.trn[,2:ncol(train.trn)]), as.matrix(train.trn$WnvPresent),
...
write.csv(submission,"lass0_baseline_1.csv",row.names = FALSE)